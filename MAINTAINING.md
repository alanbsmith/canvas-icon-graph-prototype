...
- **`GET /images/*`** (also `server/index.ts`) serves `output/images/*.png` as plain static files — added specifically for the demo app (see below) to display real icon images without needing its own copy of every PNG. Same CORS scoping as `/api`.

### The `demo/` app

A small [Vite](https://vite.dev/) + React + [Canvas Kit React](https://www.npmjs.com/package/@workday/canvas-kit-react) app for showing natural-language search to a live audience — a search box and results grid, instead of `curl` output. Deliberately its own separate npm project (own `package.json`, own `node_modules`) rather than folding React/Vite into the root project's dependencies, since the two have nothing to do with each other technically — one's a Node/Express backend, the other's a browser app — and mixing their dependency trees would only add confusion for no benefit.

A few implementation notes if this needs picking up again:
- **Only calls the REST API, never the graph or MCP directly** — same principle as the "don't query the graph database directly" decision from the original search-architecture plan, just now applying to a first-party client instead of a hypothetical third-party one.
- **Icon images are the already-rasterized PNGs**, served via the `/images/*` static route above — not Canvas Kit's own icon-component system (`SystemIcon` + statically-imported icon objects from `@workday/canvas-system-icons-web`). That system expects you to know which icon you want at *build* time (a static import); this app only knows the icon's *name* at *runtime* (from a search response), so mapping a runtime string to the right static import would need its own (fragile) name-transform logic for no real benefit over just requesting `/images/<name>.png`. `SystemIcon` IS used for the one icon that genuinely is static and known ahead of time: the search box's own magnifying-glass icon.
- **Canvas Kit's style-prop API used throughout (`padding={48}`, `gap={16}`, etc.) is technically deprecated** in favor of a newer `cs`/`createStencil` styling system — used anyway here since it's simpler to get right for a one-off demo, and "deprecated" means "there's a newer preferred way," not "broken." Worth revisiting if this app ever needs to grow past a demo prop.
- **Every Canvas Kit component/prop used was verified against the actual installed package's type definitions** before being used (same practice as the rest of this project) — several assumptions from general React-design-system experience turned out wrong on first guess (`Heading` takes `size`, but `Text`'s color prop is `variant`, not `color`; there's no separate `Card` `padding` prop, it extends `BoxProps` instead; `cs` expects a `createStyles()` result, not a plain style object). If Canvas Kit's API seems to have changed further, re-check via its actual `dist/es6/*/index.d.ts` files rather than assuming.
